// SOLUZIONE CON ALLOCAZIONE DINAMICA MEMORIA
#include<stdio.h>
#include<stdlib.h>
#include<string.h>
  ...
  else
  {
   // printf("Ho trovato il tag di chiusura %s\n", tag); // DEBUG
    ...
    if(is_last(tag, tags))
    {
      ...
      free(tags.dati[tags.index - 1]);  // libero ultimo elemento
      tags.dati = realloc(tags.dati, sizeof(char *)*(tags.index - 1)); // riduco array di stringhe
      tags.index--; // aggiorno dimensione array
      /// pop_last();
    }
    else
    {
...
void push(char *t, struct db *tags)
{
  ...
  tags->dati = realloc(tags->dati, sizeof(char *) * (tags->index +1));
